...
class Database(object):
    ...
    def execute(self, sql, args=None, autocommit=False):
        ...
        """
        #self.driver.con.ping(reconnect=True)
...
class Table(object):
    ...
    def upsert(self, record):
        ...
        :param records: 要插入的记录数据，字典or字典列表
        """
        update, p2 = format_update(record)
        if isinstance(record, dict):
            columns = record.keys()
            sql=self._get_insert_sql(columns)
            p2.update(record)
            return self.db.execute(sql+f" ON DUPLICATE KEY UPDATE {update}", p2, autocommit=True).rowcount
        else:
    ...
    def update(self, condition, update):
        ...
        p1.update(p2)
        return self.db.execute(f"update {self.name} set {update}{condition}", p1, autocommit=True).rowcount
